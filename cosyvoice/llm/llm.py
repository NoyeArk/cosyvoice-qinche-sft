...
class TransformerLM(torch.nn.Module):
    ...
    def inference(
        ...
        for i in range(max_len):
            ...
            logp = self.llm_decoder(y_pred[:, -1].to(self.llm_decoder.weight.dtype)).log_softmax(dim=-1)
...
class Qwen2Encoder(torch.nn.Module):
    ...
    def forward_one_step(self, xs, masks, cache=None):
        ...
        xs = xs.to(model_dtype)  # 确保与模型权重 dtype 一致，避免 Float vs BFloat16 错误
        outs = self.model(
            inputs_embeds=xs,
...
class Qwen2LM(TransformerLM):
    def __init__(
            ...
            length_normalized_loss: bool = True,
            lsm_weight: float = 0.0,
            ### 每输入 5 个文本 Token，预测生成 15 个语音 Token
    ...
    def prepare_lm_input_target(self, text_token, text_token_emb, text_token_len, speech_token, speech_token_emb, speech_token_len):
        """
        准备语言模型的输入和目标序列
        核心：实现N:M混合序列构建（流式）和传统序列构建（非流式）
        
        参数说明：
        - text_token: 文本token序列 [B, T_text]
        - text_token_emb: 文本token的嵌入表示 [B, T_text, D]
        - text_token_len: 每个样本的文本长度 [B]
        - speech_token: 语音token序列 [B, T_speech]
        - speech_token_emb: 语音token的嵌入表示 [B, T_speech, D]
        - speech_token_len: 每个样本的语音长度 [B]
        """
        lm_target, lm_input = [], []
        # 步骤1: 去除padding，获取真实的序列（不含padding）
        text_token = unpad_sequence(text_token, text_token_len.cpu(), batch_first=True)
        speech_token = unpad_sequence(speech_token, speech_token_len.cpu(), batch_first=True)
        ...
        for i in range(len(text_token)):
            # bistream sequence
            # self.mix_ratio[0] = N, self.mix_ratio[1] = M (如N=5, M=15)
            # 流式和非流式分别是50%的概率
            if random.random() < 0.5 and speech_token_len[i] / text_token_len[i] > self.mix_ratio[1] / self.mix_ratio[0]:
                # ========== 流式模式：N:M混合序列（bistream sequence） ==========
                this_lm_target, this_lm_input = [], []
                # IGNORE_ID=-1
                this_lm_target.append(IGNORE_ID)
                this_lm_input.append(self.llm_embedding.weight[self.sos_eos].reshape(1, -1))
                # 计算需要多少个N:M块
                # ((text_len + 1) / N)向上取整：确保覆盖所有文本
                # num_blocks = ((text_token_len[i] + 1) / self.mix_ratio[0]).ceil().int().item()
                # 处理每个块
                for j in range(((text_token_len[i] + 1) / self.mix_ratio[0]).ceil().int().item()):
                    ...
                    if len(this_text_token) == self.mix_ratio[0]:
                        ...
                        # Target构建：
                        # 1. 前N-1个文本token：忽略（因为模型应该预测填充标记）
                        this_lm_target += [IGNORE_ID] * (self.mix_ratio[0] - 1)
                        # 2. 第N个文本token位置：预测对应的语音token（这里放实际的语音token）
                    ...
                    # 最后剩下来的余数部分
                        ...
                        # 2. 不足N的文本预测出来的语音token以及后面的所有的语音token
            ...
            # unistream sequence
                ...
                # ========== 非流式模式：传统序列（unistream sequence） ==========
                # 格式：<SOS> + [全部文本token] + <TURN> + [全部语音token] + <EOS>
                
                # Target构建：
                # 1. <SOS>位置：忽略
                # 2. 所有文本token位置：忽略（因为模型应该预测填充标记？这里可能有问题）
                # 3. 所有语音token：作为预测目标
                # 4. <EOS>：作为结束标记
    ...
    def forward(
        ...
        ### 模型预测生成的语音 Token（Next Token Prediction）的准确率是多少
