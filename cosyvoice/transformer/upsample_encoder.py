...
class UpsampleConformerEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        """
        # ========== 步骤1：基础掩码和归一化 ==========
        T = xs.size(1) # time*frequency
        # 创建padding掩码：真实语音位置为1，padding位置为0
        masks = ~make_pad_mask(xs_lens, T).unsqueeze(1)  # (B, 1, T)

        # 全局CMVN（倒谱均值方差归一化）：标准化输入特征的分布
        ...
        # 每个位置可以看到更长的上下文
        # 提取多层次的特征表示
        # self.embed包含：1) 卷积下采样 2) 位置编码
        # 输入: [B, T, D], 输出: [B, T', D'], T' ≈ T/4（下采样）
        xs, pos_emb, masks = self.embed(xs, masks)
        # pos_emb: 位置编码，为Transformer提供序列顺序信息
        # masks: 下采样后的掩码

        # ========== 步骤3：处理上下文（未来信息） ==========
        if context.size(1) != 0:
            # context只在推理时提供，包含未来的一些tokens
            assert self.training is False, 'you have passed context, make sure that you are running inference mode'
        ...
        # ========== 步骤4：创建分块注意力掩码 ==========
        # use_dynamic_chunk=False, decoding_chunk_size=0
        chunk_masks = add_optional_chunk_mask(xs, masks, False, False, 0, self.static_chunk_size if streaming is True else 0, -1)
        # 结果：如果streaming=True，创建因果掩码；如果=False，使用全注意力
        # 因为默认是False，因此在训练中应该是非流式训练，也就是全注意力计算

        # ========== 步骤5：Lookahead + Conformer编码（25Hz处理） ==========
        # 1. Lookahead PreConv：提供有限未来信息
        # lookahead + conformer encoder
        xs = self.pre_lookahead_layer(xs, context=context)
        # 2. 通过多个Conformer层进行深度编码
        xs = self.forward_layers(xs, chunk_masks, pos_emb, mask_pad)
        
        # ========== 步骤6：上采样到50Hz ==========
        # upsample + conformer encoder
        xs = xs.transpose(1, 2).contiguous() # [B, D, T]
        xs, xs_lens = self.up_layer(xs, xs_lens)
        xs = xs.transpose(1, 2).contiguous() # [B, T', D]
