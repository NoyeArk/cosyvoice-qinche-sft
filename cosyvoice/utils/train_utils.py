...
def init_summarywriter(args, swan_config):
    """初始化 summary writer。当 api_key 为占位符时仅使用 tensorboard，不登录 swanlab。"""
    writer = None
    if int(os.environ.get("RANK", 0)) == 0:
        api_key = swan_config.get("api_key", "")
        use_swanlab = api_key and api_key.strip() and "your " not in api_key.lower()
        if use_swanlab:
            try:
                ...
                logging.warning(
                    "SwanLab init failed, fallback to tensorboard only: %s", e
                )
        else:
            logging.info("SwanLab api_key not configured, using tensorboard only")
...
MAX_CKPT_KEEP = 5  # 只留最新 5 个 epoch


def _cleanup_old_ckpt(model_dir, model_name_pattern, keep=MAX_CKPT_KEEP):
    """
    model_name_pattern: 形如 'epoch_*' 或 'model_*'
    按文件修改时间排序，删除最老的，只留 keep 个
    """
    ckpt_files = sorted(
        glob.glob(os.path.join(model_dir, model_name_pattern)), key=os.path.getmtime
    )
    for old in ckpt_files[:-keep]:  # 超出保留数的全部删掉
        try:
            os.remove(old)  # 删 .pt
            os.remove(re.sub(r"\.pt$", ".yaml", old))  # 连同 .yaml 一起删
...
def save_model(model, model_name, info_dict):
    ...
    if rank == 0:
        ...
        with open(info_path, "w") as fout:
            ...
            fout.write(data)
        _cleanup_old_ckpt(model_dir, "epoch_*_whole.pt", keep=MAX_CKPT_KEEP)
        logging.info(
            "[Rank 0] Checkpoint saved: %s  (auto clean-up, keep latest %d)",
            save_model_path,
            MAX_CKPT_KEEP,
        )
...
def batch_forward(model, batch, scaler, info_dict, ref_model=None, dpo_loss=None):
    ...
    with autocast:
        ...
        ### DPO相关
        ### 如果不是DPO，llm输出只有{'loss': loss, 'acc': acc}，flow的输出只有{'loss': loss}，hifigan的输出只有{'loss': loss, ...}
...
def log_per_save(writer, info_dict):
    tag = info_dict.get("tag", "CV")  # cv 调用时默认 CV，空 cv_data_loader 时 tag 可能未设置
    epoch = info_dict["epoch"]
    step = info_dict["step"]
    loss_dict = info_dict["loss_dict"]
    # lr 由 update_parameter_and_lr 设置；若 train_data_loader 为空则未设置，回退到 optim_conf 中的配置
    lr = info_dict.get("lr") or info_dict.get("optim_conf", {}).get("lr", 0)
    rank = int(os.environ.get("RANK", 0))
    logging.info(
    ...
    if writer is not None:
        ...
        writer.add_scalar("{}/lr".format(tag), lr, step + 1)
        for k, v in loss_dict.items():
            writer.add_scalar("{}/{}".format(tag, k), v, step + 1)
